...
void ModbusMaster::send_word(uint16_t data)
{
...
{
  send_word(lowWord(data));
  send_word(highWord(data));
}


void ModbusMaster::send(uint8_t data)
{
  send_word((uint16_t)data);
}
...
//#endif
    ...
    if (u32Timer.read_ms() > ku16MBResponseTimeout)
    {
    ...
    switch(u8ModbusADU[1])
    {
      ...
      case ku8MBReadDiscreteInputs:
        ...
        if (u8ModbusADU[2] % 2)
        {
          if (i < ku8MaxBufferSize)
          {
            _u16ResponseBuffer[i] = (uint16_t)(( 0 << 8 ) | u8ModbusADU[2 * i + 3]);
          }
